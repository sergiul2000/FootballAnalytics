...
def apply_pythagorian_league_table_stats(league, year):
    ...
    while y <= 2.0:
        ...
        df_simple_pythagorian, pythagorian_expectation_curent_y = formulas.compute_pythagorian_expectation(
            df_to_analyze, y)
        rmse_value = formulas.calculate_rmse(formulas.merge_lists(
            pythagorian_expectation_curent_y, df_simple_pythagorian['Points']), df_simple_pythagorian.size)
    ...
    df_simple_pythagorian = df_simple_pythagorian[[
        'Team', 'Matches', 'Wins', 'Draws', 'Loses', 'GoalsScored', 'GoalsReceived', 'Points', best_y_column]]
    df_simple_pythagorian.rename(
        columns={best_y_column: 'Estimated_Points_Simple'}, inplace=True)
    df_simple_pythagorian['Delta_Points_Simple'] = df_simple_pythagorian['Points'] - \
        df_simple_pythagorian['Estimated_Points_Simple']
    ...
    df_extended_pythagorian['AvgGS'] = df_extended_pythagorian['GoalsScored'] / \
        df_extended_pythagorian['Matches']
    df_extended_pythagorian['AvgGA'] = df_extended_pythagorian['GoalsReceived'] / \
        df_extended_pythagorian['Matches']

    # alpha GS
    df_extended_pythagorian['alphaGS'] = df_extended_pythagorian['GoalsScored'].apply(
        lambda GS: round((GS-beta)/K), 2)
    # alpha GR
    df_extended_pythagorian['alphaGA'] = df_extended_pythagorian['GoalsReceived'].apply(
        lambda GA: round((GA-beta)/K), 2)

    # alpha GS * K
    df_extended_pythagorian['alphaGGS'] = df_extended_pythagorian['alphaGS'].apply(
        lambda alphaGS: round(alphaGS * K, 2))
    # alpha GR * K
    df_extended_pythagorian['alphaGGA'] = df_extended_pythagorian['alphaGA'].apply(
        lambda alphaGA: round(alphaGA * K, 2))

    for index, row in df_extended_pythagorian.iterrows():
        # print(row)

        points_per_game_expectancy, win_expectancy, draw_expectancy = formulas.compute_points_per_game_pythagorian_estimation(
        ...
        df_extended_pythagorian.at[index,
                                   'Estimated_Wins_Extended'] = win_expectancy
        df_extended_pythagorian.at[index,
                                   'Estimated_Draws_Extended'] = draw_expectancy
        df_extended_pythagorian.at[index,
                                   'Estimated_Loses_Extended'] = lose_expectancy
        df_extended_pythagorian.at[index, 'Estimated_Points_Extended'] = points

    df_extended_pythagorian['Delta_Points_Extended'] = df_extended_pythagorian['Points'] - \
        df_extended_pythagorian['Estimated_Points_Extended']

    df_extended_pythagorian['Delta_Merge'] = df_extended_pythagorian['Points'] - \
        df_extended_pythagorian['Estimated_Points_Extended']

    print(df_extended_pythagorian)
    df_result = df_simple_pythagorian.merge(
        df_extended_pythagorian, how='right')

    df_result = df_result[['Team', 'Matches', 'Wins', 'Draws', 'Loses', 'GoalsScored', 'GoalsReceived', 'AvgGS', 'AvgGA', 'Estimated_Wins_Extended',
                           'Estimated_Draws_Extended', 'Estimated_Loses_Extended', 'Points', 'Estimated_Points_Simple', 'Delta_Points_Simple', 'Estimated_Points_Extended', 'Delta_Points_Extended']]
    ...
    mean_simple, mean_extended = analyze_differences_for_pythagorian_methods(
        df_result['Delta_Points_Simple'], df_result['Delta_Points_Extended'])
    ...
    create_empty_directories(
        'dataframes', 'pythagorian_league_table', league, year, year+1, need_year=False)
...
if __name__ == '__main__':
    ...
    for year in range(2014, 2022):
        ...
        mean_simple, mean_extended = apply_pythagorian_league_table_stats(
            'la liga', year)
    ...
    print(
        f'Avg Delta Points Extended: {mean_extended_sum / number_of_seasons}')
    print(
        f'Avg Difference Between Simple and Extended: {(mean_extended_sum - mean_simple_sum)/ number_of_seasons}')
