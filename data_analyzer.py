...
def compute_points_per_game_pythagorian_estimation(limit_for_goals_iteration, alpha_goals_scored, alpha_goals_scored_prim, alpha_goals_against, alpha_goals_against_prim, K, pythagorian_exponent):
    # compute_pythagorean_probability_for_given_goals
    #win_probability = sum(compute_pythagorean_probability_for_given_goals(goal, alpha_goals_scored, alpha_goals_against, pythagorian_exponent,True) for goal in range(0,limit_for_goals_iteration+1))
    #draw_probability = sum(compute_pythagorean_probability_for_given_goals(goal, alpha_goals_scored, alpha_goals_against, pythagorian_exponent,False)  for goal in range(0,limit_for_goals_iteration+1))
    
    # compute_pythagorean_probability
    # win_probability = sum(compute_pythagorean_probability(goal,alpha_goals_scored_prim, alpha_goals_against_prim, K, pythagorian_exponent,True) for goal in range(0,limit_for_goals_iteration + 1 ))
    # draw_probability = sum(compute_pythagorean_probability(goal,alpha_goals_scored_prim, alpha_goals_against_prim, K, pythagorian_exponent, False) for goal in range(0,limit_for_goals_iteration + 1 ))
    
    # compute_prob_1
    # win_probability = sum(compute_prob_1(goal, alpha_goals_scored, alpha_goals_against, pythagorian_exponent,True) for goal in range(0,limit_for_goals_iteration +1))
    # draw_probability = sum(compute_prob_1(goal, alpha_goals_scored, alpha_goals_against, pythagorian_exponent,False) for goal in range(0,limit_for_goals_iteration +1))
    ...
    print(f"DRAW PROB {draw_probability} ")

    return (3 * win_probability) + draw_probability , win_probability, draw_probability

    # suma_win = 0.0
    # suma_draw = 0.0
    # for goal in range(0,limit_for_goals_iteration + 1):
    #     suma_win += compute_prob_2(goal, alpha_goals_scored_prim, alpha_goals_against_prim, K, pythagorian_exponent, True)
    #     suma_draw += compute_prob_2(goal, alpha_goals_scored_prim, alpha_goals_against_prim, K, pythagorian_exponent, False)
    
    # print(f"WIN PROB {suma_win} ")
    # print(f"DRAW PROB {suma_draw} ")

    # return (-3 * suma_win) + suma_draw, suma_win, suma_draw
...
def apply_pythagorian_league_table_stats(league, year):
    ...
    for index,row in df_to_analyze.iterrows():
        ...
        points = row['Matches'] * points_per_game_expectancy
        win_expectancy = row['Matches'] * win_expectancy
        draw_expectancy = row['Matches'] * draw_expectancy
        lose_expectancy = row['Matches'] - int(win_expectancy) - (int(draw_expectancy+1))
