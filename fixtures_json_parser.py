...
def generate_fixtures_and_clean_sheets_dataframes(
    path_to_json, path_to_fixtures, path_to_clean_sheets
):
    ...
    for index, row in fixtures_df.iterrows():
        ...
        if home_goals == 0:
            ...
            clean_sheets_df.loc[
                clean_sheets_df.team_name == row["away_team"], "clean_sheets"
            ] += 1
        elif away_goals == 0:
            # print('home clean sheet')
            clean_sheets_df.loc[
                clean_sheets_df.team_name == row["home_team"], "clean_sheets"
            ] += 1
