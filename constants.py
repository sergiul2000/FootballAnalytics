...
whoscored_detailed_options = {
    ...
    "offensive": {
        "Shots": (["Zones", "Situations", "Accuracy", "Body Parts"],
                  (
            ["total_shots", "box_shots_from_outside_penalty_area",
                "inside_six_yard_box_shots", "inside_the_penalty_area_shots"],
            ["total_shots", "open_play_shots", "counter_attack_shots",
             "set_pieces_shots", "penalties_taken"],
            ["total_shots", "off_target_shots", "on_post_shots",
             "on_target_shots", "blocked_shots"],
            ["total_shots", "right_foot_shots", "left_foot_shots",
             "headers_shots", "other_body_part_shots"],
        )
        ),
        "Goals": (["Zones", "Situations", "Body Parts"],
                  (
            ["total_goals", "inside_six_yard_box_goals",
             "inside_penalty_area_goals", "outside_penalty_area_goals"],
            ["total_goals", "open_play_goals", "counter_attack_goals",
             "set_piece_goals", "penalties_scored", "own_goals", "normal_goals"],
            ["total_goals", "right_foot_goals", "left_foot_goals",
             "headers_goals", "other_goals"],
        )
        ),
    ...
    "passing": {
        "Passes": (["Length", "Type"],
                   (
            ["total_passes", "accurate_long_ball_passes", "inaccurate_long_ball_passses",
             "accurate_short_passes", "inaccurate_short_passes"],
            ["accurate_cross_passes", "inaccurate_cross_passes", "accurate_corner_passes",
             "inaccurate_corner_passes", "accurate_freekicks", "innaccurate_freekicks"]
        )
        ),
        "Key passes": (["Length", "Type"],
                       (
            ["total_key_passes", "long_passes", "short_passes"],
            ["cross_passes", "corner_passes", "throughball_passes",
             "freekick_passes", "throwin_passes", "other_passes"]
        )
        ),
