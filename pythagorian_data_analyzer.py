...
def compute_pythagorian_expectation(df_to_analyze, gamma_coeficient):
    ...
    for i in range(len(df_to_analyze)):
        ...
        ratio_win_lose.append(
            (curent_row['Wins'] + curent_row['Loses']) / curent_row['Matches'])
        ratio_draw.append(1 - ratio_win_lose[i])
        average_points_per_game.append(
            ((3 * ratio_win_lose[i]) + (2 * ratio_draw[i])))
    ...
    column_name_of_each_pythagorian_expectation_with_different_y = 'Expected Points y= ' + \
        str(gamma_coeficient)
...
def generate_formula_for_all_teams(league, year):
    ...
    while y <= 2.0:
        ...
        df_to_analyze, pythagorian_expectation_curent_y = compute_pythagorian_expectation(
            df_to_analyze, y)
        rmse_value = calculate_rmse(merge(
            pythagorian_expectation_curent_y, df_to_analyze['Points']), df_to_analyze.size)
    ...
    df_to_analyze = df_to_analyze[['Team', 'Year_start', 'Year_end', 'Matches', 'Wins', 'Draws',
                                   'Loses', 'GoalsScored', 'GoalsReceived', 'Points', best_y_column]]
    df_to_analyze.rename(
        columns={best_y_column: 'Estimated_Points_Simple'}, inplace=True)
    df_to_analyze['Delta_Points_Simple'] = df_to_analyze['Points'] - \
        df_to_analyze['Estimated_Points_Simple']
    print(df_to_analyze)
    ...
    # CEEA CE AM MODIFICAT PENTRU A AVEA TOATE ECHIPELE
...
def calculate_rmse(comparables_list, number_of_elements):
    ...
    # de verificat daca asta chiar ia dimensiunea setului, daca functioneaza ca un len
    rmse_value /= number_of_elements
