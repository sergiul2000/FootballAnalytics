...
def compute_pythagorian_expectation(df_to_analyze, gamma_coeficient):
    ...
    column_name_of_each_pythagorian_expectation_with_different_y = (
        "Expected Points y= " + str(gamma_coeficient)
    )
...
def generate_formula_for_all_teams(league, year):
    ...
    while y <= 2.0:
        ...
        (
            df_to_analyze,
            pythagorian_expectation_curent_y,
        ) = compute_pythagorian_expectation(df_to_analyze, y)
        rmse_value = calculate_rmse(
            merge(pythagorian_expectation_curent_y, df_to_analyze["Points"]),
            df_to_analyze.size,
        )
    ...
    df_to_analyze["Delta_Points_Simple"] = (
        df_to_analyze["Points"] - df_to_analyze["Estimated_Points_Simple"]
    )
    print(df_to_analyze)
