...
my_logger = logger(
    "D:\\aaLicenta\\licenta\FootballAnalytics\\utils_for_ml\\logs_of_league_table_points_regresion.txt"
)
...
df_league_table_predictions = pd.DataFrame()
...
df_league_table_predictions["Test points"] = y_test
...
DT_regressor = DecisionTreeRegressor(
    max_depth=21, min_samples_leaf=21, ccp_alpha=0.001
).fit(x_train, y_train)
...
df_league_table_predictions["Decision tree points"] = y_pred_test
...
df_league_table_predictions["Random forest points"] = y_pred_test

############################################################################################################
# XGBoost
# cu 100 de estimatoare merge cel mai bine
XGB_Regressor = XGBRegressor(
    n_estimators=100, max_depth=7, eta=0.1, subsample=0.7, colsample_bytree=0.9
).fit(x_train, y_train)
...
df_league_table_predictions["XGBoost points"] = y_pred_test
############################################################################################################
...
df_league_table_predictions["Polynomial points"] = y_pred_test
############################################################################################################
# K-fold Validation
...
df_league_table_predictions.to_csv("./converted_files/league_table_predictors.csv")
