...
def compute_pythagorean_probability(number_of_goals, alpha_goals_scored_prim, alpha_goals_against_prim, K, y, is_win):
    if is_win:
        ...
        print(
            f'1st {first_exp} 2nd {second_exp} 3rd {third_exp} 4th {fourth_exp}')
        print(f'1st {first_term} 2nd {second_term}')
        print(f'Return Value {return_val}')
...
def compute_pythagorean_probability_for_given_goals(number_of_goals, alpha_goals_scored, alpha_goals_against, pythagorian_exponent, is_win):
    if is_win:
        first_element = exp(-pow(((number_of_goals+1)/alpha_goals_scored), pythagorian_exponent)) - \
        ...
        first_element = exp(-pow(((number_of_goals+1)/alpha_goals_scored), pythagorian_exponent)) - \
            exp(-pow(((number_of_goals)/alpha_goals_scored), pythagorian_exponent))
        second_element = exp(-pow(((number_of_goals+1)/alpha_goals_against), pythagorian_exponent)) - \
            exp(-pow(((number_of_goals)/alpha_goals_against), pythagorian_exponent))
...
def compute_points_per_game_pythagorian_estimation(limit_for_goals_iteration, alpha_goals_scored, alpha_goals_scored_prim, alpha_goals_against, alpha_goals_against_prim, K, pythagorian_exponent):
    # compute_prob_2
    win_probability = sum(compute_prob_2(goal, alpha_goals_scored_prim, alpha_goals_against_prim,
                          K, pythagorian_exponent, True) for goal in range(0, limit_for_goals_iteration))
    draw_probability = sum(compute_prob_2(goal, alpha_goals_scored_prim, alpha_goals_against_prim,
                           K, pythagorian_exponent, False) for goal in range(0, limit_for_goals_iteration))
