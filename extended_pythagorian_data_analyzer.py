...
def compute_pythagorean_probability(
    number_of_goals, alpha_goals_scored_prim, alpha_goals_against_prim, K, y, is_win
):
    if is_win:
        print(
...
def compute_pythagorean_probability_for_given_goals(
    number_of_goals,
    alpha_goals_scored,
    alpha_goals_against,
    pythagorian_exponent,
    is_win,
):
    if is_win:
        first_element = exp(
            -pow(((number_of_goals + 1) / alpha_goals_scored), pythagorian_exponent)
        ) - exp(-pow(((number_of_goals) / alpha_goals_scored), pythagorian_exponent))
        second_element = 1 - exp(
            -pow((number_of_goals) / alpha_goals_against, pythagorian_exponent)
        )

        print(
            f"Win First {first_element} Second {second_element} MUL {second_element * first_element}"
        )
        ...
        second_element = exp(
            -pow(((number_of_goals + 1) / alpha_goals_against), pythagorian_exponent)
        ) - exp(-pow(((number_of_goals) / alpha_goals_against), pythagorian_exponent))
...
def compute_points_per_game_pythagorian_estimation(
    limit_for_goals_iteration,
    alpha_goals_scored,
    alpha_goals_scored_prim,
    alpha_goals_against,
    alpha_goals_against_prim,
    K,
    pythagorian_exponent,
):
    # compute_prob_2
    win_probability = sum(
        ...
        for goal in range(0, limit_for_goals_iteration)
    )
    ...
    return (
        (3 * win_probability) + (10 * draw_probability),
        win_probability,
        draw_probability * 10,
    )
...
def apply_pythagorian_league_table_stats(league, year):
    ...
    for index, row in df_to_analyze.iterrows():
        ...
        (
            points_per_game_expectancy,
            win_expectancy,
            draw_expectancy,
        ) = compute_points_per_game_pythagorian_estimation(
    ...
    df_to_analyze["Delta_Points_Extended"] = (
        df_to_analyze["Points"] - df_to_analyze["Estimated_Points_Extended"]
    )

    print(df_to_analyze)

    df_to_save = df_to_analyze[
        [
            "Team",
            # 'Year_start', 'Year_end',
            "Matches",
            ...
            "Estimated_Draws",
            "Estimated_Loses",
            "Estimated_Points_Extended",
            "Delta_Points_Extended",
        ]
    ]
...
def apply_pythagorian_league_table_stats_in_one_csv():
    ...
    # df_to_save.to_csv("initial_report.csv")
    df_to_save.to_csv(f"./converted_files/extended_pythagorean.csv")
