    ...
    for league in leagues_list:
        for filename_with_path in list_of_files[league]:
            ...
            df_iterator = df_iterator.drop(
                columns=["home_team_id", "home_short_title", "away_team_id", "away_short_title"])
            # print()
            frames.append(df_iterator)
...
def walk_through_rosters_players_and_player_summary():
    ...
    for league in leagues_list:
        for year in range(2009, 2022):
            for filename_with_path in list_of_files[league, year]:
                ...
                df_player_summary['player_number'] = df_iterator_players['player_number']
                ...
                df_player_summary['shots_per_game'] = df_iterator_players['shots_per_game']
                df_player_summary['pass_success_percentage'] = df_iterator_players['pass_success_percentage']
                df_player_summary['aerials_won'] = df_iterator_players['aerials_won']
                df_player_summary['man_of_the_match'] = df_iterator_players['man_of_the_match']
                df_player_summary['rating'] = df_iterator_players['rating']
                df_iterator_players = df_iterator_players.drop(columns=["Team", "Year_start", "Year_end", "games", "start_games", "sub_games",
    ...
    df_total_player_summary.to_csv(
        "./converted_files/player_summary_stats.csv")
    ...
    for league in leagues_list:
        for year in range(2009, 2022):
            for filename_with_path in list_of_files[league, year]:
                ...
                df_iterator_players = df_iterator_players.drop(columns=["games", "start_games", "sub_games", "name", "age",
                                                                        "total_assists", "pass_success_percentage",
                                                                        "tall", "position", "weight",
                                                                        "mins"])  # , "rating"])
