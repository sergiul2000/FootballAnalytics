...
def crawl_chosen_stats_between_years(
    league, teams_details, start_year, end_year, stat="summary"
):
    ...
    now_total = time.time()
    logging.basicConfig(
        filename=f"debugger_logs/debugger_time_crawling_for_{stat}.log",
        encoding="utf-8",
        level=logging.INFO,
        filemode="a",
    )
    ...
    create_empty_directories(
        "dataframes",
        f"whoscored_player_{stat}_stats",
        league,
        start_year,
        end_year,
        True,
    ...
    for team, code in teams_details.items():
        dict_of_league_urls = crawl_all_urls_for_given_team_in_league_competitions(
            code, league, first_season_year=start_year, last_season_year=end_year
        )
        ...
        for item in dict_of_league_urls.items():
            ...
            match stat:
                case "summary":
                    ...
                    save_stats_csv(
                        df_season_summary_stats,
                        "summary",
                        league,
                        team,
                        season_start_year=int(item[0].split("_")[1]),
                    )
                case "offensive":
                    df_season_offensive_stats = crawl_player_team_stats_offensive(
                        item[1], is_current_season
                    )
                    is_current_season = False

                    save_stats_csv(
                        df_season_offensive_stats,
                        "offensive",
                        league,
                        team,
                        season_start_year=int(item[0].split("_")[1]),
                ...
                case "defensive":
                    df_season_defensive_stats = crawl_player_team_stats_defensive(
                        item[1], is_current_season
                    )
                    is_current_season = False

                    save_stats_csv(
                        df_season_defensive_stats,
                        "defensive",
                        league,
                        team,
                        season_start_year=int(item[0].split("_")[1]),
                ...
                case "passing":
                    df_season_passing_stats = crawl_player_team_stats_passing(
                        item[1], is_current_season
                    )
                    is_current_season = False

                    save_stats_csv(
                        df_season_passing_stats,
                        "passing",
                        league,
                        team,
                        season_start_year=int(item[0].split("_")[1]),
                ...
                case "detailed_offensive":
                    df_detailed_offensive = crawl_player_team_stats_detailed(
                        item[1], "offensive", is_current_season
                    )
                    is_current_season = False

                    save_stats_csv(
                        df_detailed_offensive,
                        "detailed_offensive",
                        league,
                        team,
                        season_start_year=int(item[0].split("_")[1]),
                ...
                case "detailed_defensive":
                    df_detailed_defensive = crawl_player_team_stats_detailed(
                        item[1], "defensive", is_current_season
                    )
                    is_current_season = False

                    save_stats_csv(
                        df_detailed_defensive,
                        "detailed_defensive",
                        league,
                        team,
                        season_start_year=int(item[0].split("_")[1]),
                ...
                case "detailed_passing":
                    df_detailed_passing = crawl_player_team_stats_detailed(
                        item[1], "passing", is_current_season
                    )
                    is_current_season = False
    ...
    driver_connection_time_average += float(driver_connection_time_sum) / float(
        driver_connection_counter
    )
    fetching_data_time_average += float(fetching_data_time_sum) / float(
        fetching_data_counter
    )

    print(
        f"[TIME LOG DRIVER CONNECTION Average Time for {stat.upper()} in league {league.upper()}] - On Average the Process for league {league} took {float(driver_connection_time_sum) / float(driver_connection_counter)} seconds per team"
    )
    logging.info(
        f"[TIME LOG DRIVER CONNECTION Average Time for {stat.upper()} in league {league.upper()}] - On Average the Process for league {league} took {float(driver_connection_time_sum) / float(driver_connection_counter)} seconds per team"
    )
    ...
    print(
        f"[TIME LOG FETCHING DATA Average Time for {stat.upper()} in league {league.upper()}] - On Average the Process for league {league} took {float(fetching_data_time_sum) / float(fetching_data_counter)} seconds per team"
    )
    logging.info(
        f"[TIME LOG FETCHING DATA Average Time for {stat.upper()} in league {league.upper()}] - On Average the Process for league {league} took {float(fetching_data_time_sum) / float(fetching_data_counter)} seconds per team"
    )
    ...
    print(
        f"[TIME LOG FINAL Crawling for {stat.upper()} for league {league.upper()}] - Whole Process took for league {league} took {difference_total} seconds"
    )
    logging.info(
        f"[TIME LOG FINAL Crawling for {stat.upper()} for league {league.upper()}] - Whole Process took for league {league} took {difference_total} seconds"
    )
...
def crawl_all_urls_for_given_team_in_league_competitions(
    team_id, competition_name, first_season_year=2009, last_season_year=2023
):
    ...
    if last_season_year == 2023:
        ...
        dict_of_league_competitions_urls[
            f"{competition_name}_{last_season_year}_{last_season_year+1}"
        ] = url_current_season
    ...
    for option in select.options:
        ...
        competition_text, season_text_start = transform_competition_name(
            competition_name
        )
...
def establish_driver_connection(
    url,
    statistic="summary",
    button_xpath='//a[contains(@href,"#team-squad-archive-stats-offensive")]',
    api_delay_term=API_DELAY_FOR_YOUR_PC,
):
    """
    ...
    service = Service("exe_dependencies/chromedriver")

    # Pass the Service object when creating the WebDriver
    driver = webdriver.Chrome(service=service)

    # driver = webdriver.Chrome(executable_path="exe_dependencies/chromedriver")  linia asta era, decomentata e ca nou
    ...
    logging.info(
        f"[TIME LOG 1] - Establishing Driver Connection took {difference} seconds"
    )
...
def create_crawler_empty_df(list_of_extra_columns, are_sub_games_existent=False):
    ...
    list_of_columns.extend(
        ["start_games", "sub_games", "mins"] if are_sub_games_existent else ["mins"]
    )
...
def create_dictionary_row_for_crawler(
    element, list_of_extra_columns, are_sub_games_existent=False
):
    # start of the process
    # now = time.time()

    player_dict = {
        "player_number": element.find_elements("css selector", "td")[0]
        .find_elements("css selector", "a")[0]
        .get_attribute("href")
        .split("/")[4],
        "name": element.find_elements("css selector", "td")[0]
        .find_elements("css selector", "a")[0]
        .find_elements("css selector", "span")[0]
        .text,
    ...
    for entry in list_of_extra_columns:
        player_dict[entry] = element.find_elements("css selector", "td")[
            table_data_index
        ].text
        table_data_index += 1

    player_dict["rating"] = element.find_elements("css selector", "td")[
        table_data_index
    ].text
    # print(player_dict)
...
def crawl_player_team_stats_summary(url):
    ...
    logging.basicConfig(
        filename="debugger_logs/debugger_time_summary.log",
        encoding="utf-8",
        level=logging.INFO,
        filemode="a",
    )
    ...
    player_summary_df = create_crawler_empty_df(
        list_of_extra_columns=who_scored_simple_columns["summary"],
        are_sub_games_existent=True,
    )
    ...
    for element in elements:
        player_dict = create_dictionary_row_for_crawler(
            element,
            list_of_extra_columns=who_scored_simple_columns["summary"],
            are_sub_games_existent=True,
        )
    ...
    logging.info(
        f"[TIME LOG 3 SUMMARY] - Fetching Player Data took {difference} seconds"
    )
    ...
    logging.info(
        f"[TIME LOG FINAL SUMMARY] - Whole Process took {difference_total} seconds"
    )
...
def crawl_player_team_stats_offensive(url, is_current_season=False):
    ...
    now_total = time.time()
    logging.basicConfig(
        filename="debugger_logs/debugger_time_offensive.log",
        encoding="utf-8",
        level=logging.INFO,
        filemode="a",
    )
    ...
    if is_current_season:
        driver = establish_driver_connection(
            url, "offensive", '//a[contains(@href,"#team-squad-stats-offensive")]'
        )
    else:
        driver = establish_driver_connection(
            url,
            "offensive",
            '//a[contains(@href,"#team-squad-archive-stats-offensive")]',
        )

    player_offensive_df = create_crawler_empty_df(
        list_of_extra_columns=who_scored_simple_columns["offensive"],
        are_sub_games_existent=True,
    )
    ...
    for element in elements:
        if len(element.find_elements("css selector", "td")) < 16:
            continue
    ...
    logging.info(
        f"[TIME LOG 3 OFFENSIVE] - Fetching Player Data took {difference} seconds"
    )
    ...
    logging.info(
        f"[TIME LOG FINAL OFFENSIVE] - Whole Process took {difference_total} seconds"
    )
...
def crawl_player_team_stats_defensive(url, is_current_season=False):
    ...
    now_total = time.time()
    logging.basicConfig(
        filename="debugger_logs/debugger_time_defensive.log",
        encoding="utf-8",
        level=logging.INFO,
        filemode="a",
    )
    ...
    if is_current_season:
        driver = establish_driver_connection(
            url, "defensive", '//a[contains(@href,"#team-squad-stats-defensive")]'
        )
    else:
        driver = establish_driver_connection(
            url,
            "defensive",
            '//a[contains(@href,"#team-squad-archive-stats-defensive")]',
        )

    player_defensive_df = create_crawler_empty_df(
        list_of_extra_columns=who_scored_simple_columns["defensive"],
        are_sub_games_existent=True,
    )
    ...
    for element in elements:
        if len(element.find_elements("css selector", "td")) < 15:
            continue
    ...
    logging.info(
        f"[TIME LOG 3 DEFENSIVE] - Fetching Player Data took {difference} seconds"
    )
...
def crawl_player_team_stats_passing(url, is_current_season=False):
    ...
    now_total = time.time()
    logging.basicConfig(
        filename="debugger_logs/debugger_time_passing.log",
        encoding="utf-8",
        level=logging.INFO,
        filemode="a",
    )
    ...
    if is_current_season:
        driver = establish_driver_connection(
            url, "passing", '//a[contains(@href,"#team-squad-stats-passing")]'
        )
    else:
        driver = establish_driver_connection(
            url, "passing", '//a[contains(@href,"#team-squad-archive-stats-passing")]'
        )

    player_passing_df = create_crawler_empty_df(
        list_of_extra_columns=who_scored_simple_columns["passing"],
        are_sub_games_existent=True,
    )
    ...
    for element in elements:
        if len(element.find_elements("css selector", "td")) < 14:
            continue
    ...
    logging.info(
        f"[TIME LOG 3 PASSING] - Fetching Player Data took {difference} seconds"
    )
    ...
    logging.info(
        f"[TIME LOG FINAL PASSING] - Whole Process took {difference_total} seconds"
    )
...
def establish_driver_connection_for_detailed_section(
    url,
    xpath_for_current_or_archive,
    stats_selection="Shots",
    is_circumstance_needed=True,
    circumstance_selection_text="Zones",
    stats_period_selection="Total",
    api_delay_term=API_DELAY_FOR_YOUR_PC,
):
    ...
    if driver_singleton == None:
        driver_singleton = establish_driver_connection(
            url, "detailed", xpath_for_current_or_archive
        )
    ...
    print(
        f"[TIME LOG 1.1] - Establishing Detailed Driver Connection took {difference} seconds"
    )
    logging.info(
        f"[TIME LOG 1.1] - Establishing Detailed Driver Connection took {difference} seconds"
    )
...
def crawl_player_team_stats_detailed(
    url, category="offensive", is_current_season=False
):
    global driver_singleton
    ...
    for subcategory, conditions in subcategory_mapping.items():
        ...
        if conditions[0] == []:
            ...
            if is_current_season:
                driver = establish_driver_connection_for_detailed_section(
                    url,
                    '//a[contains(@href,"#team-squad-stats-detailed")]',
                    subcategory,
                    is_circumstance_needed=False,
                    stats_period_selection="Total",
                )
            else:
                driver = establish_driver_connection_for_detailed_section(
                    url,
                    '//a[contains(@href,"#team-squad-archive-stats-detailed")]',
                    subcategory,
                    is_circumstance_needed=False,
                    stats_period_selection="Total",
                )

            player_detailed_df = create_crawler_empty_df(
                columns, are_sub_games_existent=False
            )
            ...
            elements = WebDriverWait(
                driver, 2.0, ignored_exceptions=ignored_exceptions
            ).until(
                expected_conditions.presence_of_element_located(
                    (By.CSS_SELECTOR, "#player-table-statistics-body tr")
                )
            )

            elements = driver.find_elements(
                "css selector", "#player-table-statistics-body tr"
            )
            ...
            print(
                f"[TIME LOG 3 DETAILED {category.upper()} | {subcategory.upper()}] - Fetching Player Data took {difference} seconds"
            )
            logging.info(
                f"[TIME LOG 3 DETAILED {category.upper()} | {subcategory.upper()}] - Fetching Player Data took {difference} seconds"
            )
            ...
            # Remove Nan value rows
            player_detailed_df = remove_nan_values_added_while_scrapping(
                player_detailed_df
            )
            ...
            for characteristic in characteristics:
                ...
                if is_current_season:
                    driver = establish_driver_connection_for_detailed_section(
                        ...
                        subcategory,
                        is_circumstance_needed=True,
                        circumstance_selection_text=characteristic,
                        stats_period_selection="Total",
                    )
                else:
                    driver = establish_driver_connection_for_detailed_section(
                ...
                player_detailed_df = create_crawler_empty_df(
                    columns[index_columns], are_sub_games_existent=False
                )
                ...
                for element in elements:
                    player_dict = create_dictionary_row_for_crawler(
                        element, columns[index_columns], False
                    )
                ...
                print(
                    f"[TIME LOG 3 DETAILED {category.upper()} | {subcategory.upper()} - {characteristic.upper()} ] - Fetching Player Data took {difference} seconds"
                )
                logging.info(
                    f"[TIME LOG 3 DETAILED {category.upper()} | {subcategory.upper()} - {characteristic.upper()}] - Fetching Player Data took {difference} seconds"
                )
    ...
    for df in list_df:
        ...
        if player_detailed_df is None:
            ...
            player_detailed_df = player_detailed_df[
                player_detailed_df.columns.drop(
                    list(player_detailed_df.filter(regex="Unnamed"))
                )
            ]
    # print('FINAL DF')
    ...
    print(
        f"[TIME LOG FINAL DETAILED {category.upper()}] - Whole Process took {difference_total} seconds"
    )
    logging.info(
        f"[TIME LOG FINAL DETAILED {category.upper()}] - Whole Process took {difference_total} seconds"
    )
    ...
    logging.info(
        f"[LOG DETAILED {category.upper()}] - {category.capitalize()} Fetching Done"
    )
...
if __name__ == "__main__":
    ...
    # BARCELONA DETAILED PASSING STATS
    # df = crawl_player_team_stats_detailed(
    #     "https://www.whoscored.com/Teams/65/Archive/?stageId=19895", "passing", False
    # )

    # SEVILIA DETAILED PASSING STATS
    # df = crawl_player_team_stats_detailed(
    #     "https://www.whoscored.com/Teams/67/Archive/?stageId=3218", "passing", False
    # )

    df.to_csv("demo\\WhoScored\\scraping_demo_df.csv")
